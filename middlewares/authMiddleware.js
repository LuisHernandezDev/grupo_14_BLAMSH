...
const middlewares = {
    ...
    isAdmin: (req, res, next) => {
        ...
        const isAdmin = user && user.email === 'lilifotis3@gmail.com'; // Middleware local. Verificamos si el usuario es administrador
    ...
    guestUser: (req, res, next) => { // Si la sesión esta iniciada, redirecciona a profile y no permitas ir a register
        if (req.session.user && req.session.user.email === 'lilifotis3@gmail.com') {
            return next();
